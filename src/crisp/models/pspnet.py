...
def conv3x3(in_planes, out_planes, stride=1, dilation=1):
    """Credit: https://github.com/OasisYang/Wild6D/"""
    return nn.Conv2d(
...
class PSPUpsample(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, out_channels, 3, padding=1), nn.PReLU())
...
class PSPNet(nn.Module):
    """PSPNet with adjustable convolutional backbone provided by TIMM
    Based on the PSPNet layers used in Wild6D
    """

    def __init__(
        self,
        bins=(1, 2, 3, 6),
        input_im_dims=(1, 3, 256, 256),
        backbone="resnet18",
        use_pretrained_backbone=True,
        freeze_backbone=True,
    ):
        super().__init__()
        assert math.log2(input_im_dims[-1]).is_integer()

        # create backbone using TIMM
        # output map dimension will be fixed to 32
        self.feats = timm.create_model(
            backbone,
            pretrained=use_pretrained_backbone,
            features_only=True,
            output_stride=input_im_dims[-1] // 32,
            out_indices=(-1,),
        )
        feat_dim = self.feats.feature_info.info[-1]["num_chs"]

        if freeze_backbone:
            print("PSPNet will freeze backbone weights.")
            if freeze_backbone:
    ...
    def forward(self, x):
        # f shape: torch.Size([10, 512, 32, 32])
        f = self.feats(x)[-1]
        p = self.psp(f)
        p = self.up_1(p)
        p = self.drop(p)
        ...
        p = self.drop(p)
        p = self.up_3(p)
        return f, self.final(p)


class PSPNet_ResNet18(nn.Module):
    """Credit: https://github.com/OasisYang/Wild6D/"""
