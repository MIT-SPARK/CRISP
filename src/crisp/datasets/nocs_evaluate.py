...
def compute_combination_mAP(
    final_results, synset_names, degree_thresholds=[5, 10, 15], shift_thresholds=[0.1, 0.2], iou_3d_thresholds=[0.1]
):
...
def compute_independent_mAP(
    ...
    final_results_msg = "3D IoU at 25: {:.1f}".format(iou_3d_aps[-1, iou_thres_list.index(0.25)] * 100) + "\n"
    final_results_msg += "3D IoU at 50: {:.1f}".format(iou_3d_aps[-1, iou_thres_list.index(0.5)] * 100) + "\n"
    final_results_msg += "3D IoU at 75: {:.1f}".format(iou_3d_aps[-1, iou_thres_list.index(0.75)] * 100) + "\n"

    final_results_msg += (
        "5 degree, 2cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(2)] * 100) + "\n"
    )
    final_results_msg += (
        "5 degree, 5cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(5)] * 100) + "\n"
    )
    final_results_msg += (
        "5 degree, 10cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(10)] * 100)
        + "\n"
    )

    final_results_msg += (
        "10 degree, 2cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(2)] * 100)
        + "\n"
    )
    final_results_msg += (
        "10 degree, 5cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(5)] * 100)
        + "\n"
    )
    final_results_msg += (
    ...
    return iou_3d_aps, pose_aps, final_results_msg


def evaluate(path, artifacts_save_dir):
    ...
    iou_3d_aps, pose_aps, final_results_msg = compute_independent_mAP(
        final_results,
        synset_names,
    ...
    # save results to arfifacts path
    with open(os.path.join(artifacts_save_dir, "final_results.pkl"), "wb") as f:
        cPickle.dump({"iou_3d_aps": iou_3d_aps, "pose_aps": pose_aps, "final_results": final_results_msg}, f)
