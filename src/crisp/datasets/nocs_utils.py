...
from crisp.datasets.nocs_align import estimateSimilarityTransform
...
def obj_name_from_path(obj_model_path: Path):
    """Automatically determine the object label from the path;
    handling both the NOCS real subset and camera subset.
    """
    pparts = Path(obj_model_path).parts
    if "real_train" in pparts:
        subset = "real_train"
    elif "real_test" in pparts:
        subset = "real_test"
    elif "camera_train" in pparts:
        subset = "camera_train"
    elif "camera_val" in pparts:
        subset = "camera_val"
    else:
        raise ValueError(f"Unclear subset for object model path: {obj_model_path}")

    if "real" in subset:
        # real subset
        obj_label = pparts[-1].split(".")[0]
    else:
        # camera subset
        obj_label = f"{pparts[-3]}_{pparts[-2]}"
    return obj_label, subset
...
def extract_inst_bboxes(inst_dict, mask):
    """Compute bounding boxes from masks, indexed by their instance IDs.
    Returns: dictionary, key=instance ID, value=bbox array (y1, x1, y2, x2)
    """
    boxes = {}
    for inst_id in inst_dict.keys():
        m = np.equal(mask, inst_id)
        # Bounding box.
        ...
        if horizontal_indicies.shape[0]:
            ...
            x1, x2, y1, y2 = 0, 0, 0, 0
        boxes[inst_id] = np.array([y1, x1, y2, x2])
    return boxes
...
def compute_3d_iou(bbox_3d_1, bbox_3d_2, handle_visibility, class_name_1, class_name_2):
    ...
    if symmetry_flag:
        ...
        y_rotation_matrix = np.array([[np.cos(theta), 0, np.sin(theta)], [0, 1, 0], [-np.sin(theta), 0, np.cos(theta)]])
...
def minimize_mask(bbox, mask, mini_shape):
    ...
    # for coordinate map
        ...
        for i in range(mask.shape[-2]):
            ...
            y1, x1, y2, x2 = bbox[i][:4]
            m = m[y1:y2, x1:x2, :] * 255
            m = resize(m.astype(np.uint8), mini_shape + (mask.shape[-1],))
...
def expand_mask(bbox, mini_mask, image_shape):
    ...
    if len(mini_mask.shape) == 3:
        ...
        for i in range(mask.shape[-2]):
            ...
            h = y2 - y1
            w = x2 - x1
            m = resize(m.astype(float), (h, w, mini_mask.shape[-1]))
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j] = compute_RT_upper_bound_symmetry(
                pred_RTs[i], gt_RTs[j], gt_class_ids[j], synset_names, thres_shift=thres_shift, thres_theta=thres_degree
            )
...
def draw_text(draw_image, bbox, text, draw_box=False):
    ...
    if draw_box:
        cv2.rectangle(draw_image, (bbox[1], bbox[0]), (bbox[3], bbox[2]), (255, 0, 0), 2)
