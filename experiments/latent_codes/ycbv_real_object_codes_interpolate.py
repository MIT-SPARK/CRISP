...
class TestMode(Enum):
    ...
    EXTRAPOLATE = 2
    SIMPLEX_SAMPLE = 3
    EXTRAPOLATE_BETWEEN_SPECIFIED_OBJS = 4
    SAVE_RANDOM_MESH = 5
...
def main(opt):
    ...
    shp_code_save_folder = os.path.join(f"./data/ycbv_{model_name}")
    raw_obj_code_db = {}
    if os.path.exists(os.path.join(shp_code_save_folder, "raw_obj_codes.npy")):
        ...
        for i, batch in tqdm(enumerate(dl), total=len(dl)):
            # nocs related
            rgb_img, segmap, gt_nocs, coords, gt_normals, gt_sdf, metadata = (
                batch["rgb"].cuda(),
            ...
            for j in range(bs):
                if metadata["label"][j] not in raw_obj_code_db.keys():
                    raw_obj_code_db[metadata["label"][j]] = [shape_code[j, ...].numpy(force=True)]
                else:
                    raw_obj_code_db[metadata["label"][j]].append(shape_code[j, ...].numpy(force=True))
    ...
    if TEST_MODE == TestMode.SIMPLEX_SAMPLE:
        ...
        save_mesh_from_shape_codes(
            model, shape_code_dict, export_folder=f"./exports/ycbv/train/random_meshes_{model_name}/"
        )
