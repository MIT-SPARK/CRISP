...
runs_folder = script_dir.parent / "exp_results" / "20241101_sota_comps"
...
def plot_adds_cdf(dfs, column="adds_to_plot", dump_folder=pathlib.Path("./"), xlim_max=0.06):
    ...
    fig, ax = plt.subplots(1, 1)
    sns.ecdfplot(data=df, x=column, hue="method", ax=ax, palette=name2color)
...
def plot_adds_boxplot(dfs, column="adds_to_plot", dump_folder=pathlib.Path("./")):
    ...
    plt.savefig(dump_folder / f"ycbv_test_adds_boxplot_{RAND_STRING}.{FILE_EXTENSION}", dpi=200)
    return
...
def plot_shape_chamfer_boxplot(dfs_to_plot_list, column="shape_chamfer", dump_folder=pathlib.Path("./")):
    ...
    plt.savefig(dump_folder / f"ycbv_test_shape_chamfer_boxplot_{RAND_STRING}.{FILE_EXTENSION}", dpi=200)
...
if __name__ == "__main__":
    ...
    # make table
    total_pose_df = pd.concat(pose_dfs_to_plot_list, ignore_index=True)
    adds_metric_df = make_pose_add_s_table(total_pose_df)
    adds_metric_df.to_csv(os.path.join(script_dir.parent / "plots", f"adds_metrics_{RAND_STRING}.csv"), index=False)

    total_shape_df = pd.concat(shape_chamfer_dfs_to_plot_list, ignore_index=True)
    shape_chamfer_df = make_shape_table(total_shape_df)
    shape_chamfer_df.to_csv(
        os.path.join(script_dir.parent / "plots", f"shape_chamfer_metrics_{RAND_STRING}.csv"), index=False
    )
